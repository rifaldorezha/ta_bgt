...
module.exports = {
  ...
  getPangkas_pohonByID: async (req, res) => {
    ...
    try {
      if (!p1) {
        res.status(404).json({
          message: "Could not Found pangkas pohon by id",
        });
      } else {
        res.status(200).send({
          status: "Success",
          message: "Get Pangkas pohon with id",
          data: p1,
        });
      }
    } catch (error) {
  ...
  addPangkas_pohon: async (req, res) => {
    ...
    try {
      ...
      res.status(200).send({
        status: "Success",
        message: "add data Pangkas pohon berhasil",
        data: { pangkasId },
      });
    } catch (error) {
      console.log(error);
      res.status(500).send({
        status: "failed",
        message: "Pangkas pohon add invalid",
      });
    }
  },
  ...
  updatePangkas_pohonByID: async (req, res) => {
    ...
    try {
      ...
      res.status(200).send({
        status: "Success",
        message: "resource has successfully updated Pangkas pohon data",
        data: {
          pohonId,
        },
      });
